...
router.post("/", apiHelpers.vaildateRegisterData, (req, res) => {
	...
	mongooseHelpers
		.createUser(username, email, password, title, readAccess, writeAccess)
		.then((result) => {
	...
	mongooseHelpers.updateUser(userId, updateBody).then((result) => {
		if (result.nModified > 0) {
			...
			apiHelpers.sendStatus(
				...
				"error",
				null,
				"Successful request but data already exists",
				req,
				res
			);
		}
...
router.post(
	"/:userid/addresses",
	apiHelpers.addressExists,
	apiHelpers.validQueryChecker(["shipping-address", "address"]),
	apiHelpers.userExists,
	(req, res) => {
		const userId = req.result;
		const updateBody = req.body;
		...
		mongooseHelpers.createAddressForUser(userId, newUpdateBody).then((result) => {
			if (result.nModified > 0) {
				apiHelpers.sendStatus(
					...
					"success",
					result,
					"Address details successfully created",
					req,
					res
				);
			} else {
